...
\begin{align*}
    ...
    &= \nabla_{x_i} \left( \frac{1}{2} \sum_{k=1}^{n} \sum_{j \ne k} w_{kj} \left(d_{kj} - \|x_k - x_j\|\right)^2 \right)\\
\end{align*}
Because the distance function $||x_k - x_j||$ and the weights $w_{kj}$ are symmetric , the derivative of the terms where $k=i$ is identical to the derivative of the terms where $j=i$
\begin{align*}
    \nabla_{x_i} E(x) &= \frac{1}{2} \sum_{i \ne j} w_{ij}  \nabla_{x_i} \left( \left(d_{ij} - \|x_i - x_j\|\right)^2 \right)\\
...
\end{align*}
The 2 comes from the fact that each pair $(i,j)$ contributes to the gradient twice when we differentiate with respect to $x_i$.\\
\end{document}